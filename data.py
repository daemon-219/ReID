...
class Data():
    def __init__(self):
        ...
        self.trainset = Market1501(train_transform, 'train', opt.data_path, opt.fake_ratio)
        self.testset = Market1501(test_transform, 'test', opt.data_path)
        self.queryset = Market1501(test_transform, 'query', opt.data_path)

        self.real_trainset = Market1501(train_transform, 'train', opt.data_path, 0.0)
        self.fake_trainset = Market1501(train_transform, 'train', opt.data_path, 1.0)
        ...
        self.real_train_loader = dataloader.DataLoader(self.real_trainset,
                                                  sampler=RandomSampler(self.real_trainset, batch_id=opt.batchid,
                                                                        batch_image=opt.batchimage),
                                                  batch_size=opt.batchid * opt.batchimage, num_workers=8,
                                                  pin_memory=True)
        self.fake_train_loader = dataloader.DataLoader(self.fake_trainset,
                                                  sampler=RandomSampler(self.fake_trainset, batch_id=opt.batchid,
                                                                        batch_image=opt.batchimage),
                                                  batch_size=opt.batchid * opt.batchimage, num_workers=8,
                                                  pin_memory=True)
...
class Market1501(dataset.Dataset):
    def __init__(self, transform, dtype, data_path, fake_ratio=0.0):

        self.transform = transform
        self.loader = default_loader
        self.data_path = data_path
        # fake images for training
        self.fake_path = data_path
        self.fake_ratio = fake_ratio

        if fake_ratio > 0:
            self.fake_path += '/synthesis_train'
        ...
        self.fake_imgs = [path for path in self.list_pictures(self.fake_path) if self.id(path) != -1]
    ...
    def __getitem__(self, index):
        ...
        # use fake images
        if self.fake_ratio > torch.rand(1):
            path = self.fake_imgs[index]
